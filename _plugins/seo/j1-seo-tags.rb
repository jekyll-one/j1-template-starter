...
module Jekyll
  ...
  class J1SeoTag < Jekyll::Generator
    ...
    # used a 5-line if/else; the negated boolean read is clearer and
    # fails safe (missing config => disabled).
    #
    def plugin_disabled?
      !config['enabled']
    end
    ...
    def load_yaml(path)
      ...
      Jekyll.logger.warn 'J1 SEO Tags:', "failed to read #{path}: #{e.message}"
      {}
    end
  ...
  #
  # The original class body performed file-system path resolution and
  # YAML parsing AT CLASS-BODY LOAD TIME, i.e. as soon as Ruby required
  # the plugin file. That had several problems:
  #
  ...
  #
  # The fix is to defer all of (1)-(4) to first-render time, when a
  # Liquid context (and therefore `context.registers[:site].source`) is
  # available. A class-level memo (`SeoTag.site_source`) is set on the
  # very first `render` call and reused thereafter.
  #
  class SeoTag < Liquid::Tag
    ...
    def payload
      ...
      # See https://github.com/jekyll/jekyll/blob/22f2724a1f117a94cc16d18c499a93d5915ede4f/lib/jekyll/site.rb#L261-L276
      #
    ...
    class << self
      # Site source is captured at first render (see `render` above) and
      # used to resolve plugin/template paths lazily.
      #
      ...
      # Resolve the template file lazily, using the site source captured
      # at first render. The original computed this at class-body load
      # time using a fragile __FILE__ substitution.
      #
      def template_path
        @template_path ||= File.expand_path(module_config['template_name'].to_s, module_template_path)
      end

      def module_template_path
        File.join(site_source.to_s, '_data', 'templates')
      end

      # Lazy, safe, non-destructive load of the plugin config. Replaces
      # the eager class-body load that ran before any site existed.
      #
      def module_config
        @module_config ||= begin
          plugins_dir = File.join(site_source.to_s, '_data', 'plugins')
          defaults    = load_yaml(File.join(plugins_dir, 'defaults', 'seo-tags.yml'))['defaults'] || {}
          user        = load_yaml(File.join(plugins_dir, 'seo-tags.yml'))['settings']             || {}
          defaults.merge(user)
        end
      end
...
module Jekyll
  class SeoTag
    ...
    class Drop < Jekyll::Drops::Drop
      ...
      # Bug fix: original wrote to @site_title_extention, which is the
      # ivar used by `site_title_extention` above. Calling
      # `page_title_extention` first poisoned the site-level cache with
      # the page's value (and any subsequent call returned the page
      # value when asked for the site value). The correct ivar is
      # @page_title_extention.
      #
      def page_title_extention
        @page_title_extention ||= format_string page["title_extention"]
      end
      ...
      # Removed the redundant trailing `@title` line - `||=` already
      # returns the cached/assigned value, so the explicit return was
      # dead code.
      ...
      # but `current_time.to_liquid` is the string returned by
      # strftime, which is always truthy, so `page["last_modified"]`
      # was never consulted. The intended precedence is the page's
      # explicit `last_modified` front-matter, falling back to the
      # current build time. Also dropped `Time.new` + `strftime` round
      # trip - `date_to_xmlschema` accepts a Time directly, and the
      # filter formats it consistently with `date_published`.
      #
      def date_modified
        @date_modified ||= begin
          date = page["last_modified"] || Time.now
...
module Jekyll
  class SeoTag
    class JSONLDDrop < Jekyll::Drops::Drop
      ...
      # Expose #type and #logo as private methods and #@type
      # as a public method.
      #
